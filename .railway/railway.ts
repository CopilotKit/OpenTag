...
export function createOpenTagProject(
  ...
  const datadog = createDatadogTopology(context, {
    ...
    branch: BRANCH,
    targets: {
      runtimeServiceId: runtime.env.RAILWAY_SERVICE_ID,
      agentServiceId: agent.env.RAILWAY_SERVICE_ID,
    },
  });
...
export default defineRailway((context) => createOpenTagProject(context));
