/**
 * SQLite persistence backend for TanStack AI, adapted from the official
 * worked example:
 *   TanStack/ai examples/ts-react-chat/src/lib/sqlite-persistence.ts
 *   docs: docs/persistence/build-your-own-adapter.md
 *
 * Built on `@tanstack/ai-persistence` store contracts + Node's built-in
 * `node:sqlite` driver. No ORM. No extra packages.
 *
 * Provides all seven stores (chat: messages/runs/interrupts/metadata;
 * generation: generationRuns/artifacts/blobs). Hand to
 * `withPersistence(...)` / `withGenerationPersistence(...)`, and pass
 * `stores.runs` into `withSandbox(..., { runs, durability })`.
 *
 * Semantics match the in-memory reference backend. Prove changes with
 * `runPersistenceConformance` from `@tanstack/ai-persistence/testkit`.
 *
...
import { createSqliteInstanceStore } from './sqlite-instance-store.js'
 ...
 * also exposes `close()` to release the file handle, plus OpenTag extras:
 * `instances` (sandbox instance rows) and `findLatestRun` (newest run for a
 * thread, including finished ones).
 *
 ...
 */
export type SqlitePersistenceHandle = SqliteAIPersistence & {
  close: () => void
  instances: SandboxInstanceStore
  findLatestRun: (threadId: string) => Promise<RunRecord | null>
}

export function sqlitePersistence(
  options: SqlitePersistenceOptions,
): SqlitePersistenceHandle {
  ...
  return {
    ...persistence,
    instances: createSqliteInstanceStore(db),
    findLatestRun(threadId) {
      const row = latestRunStmt.get(threadId) as RunRow | undefined
      return Promise.resolve(row ? mapRun(row) : null)
    },
    close() {
      if (closed) return
